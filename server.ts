...
async function startServer() {
  ...
  app.post("/api/update-nfc-view", (req, res) => {
    try {
      const { url } = req.body;
      if (!url) {
      ...
      const nfcViewPath = path.join(__dirname, "nfc-view.html");
      if (fs.existsSync(nfcViewPath)) {
        let content = fs.readFileSync(nfcViewPath, "utf-8");
        // Replace the iframe src more robustly
        content = content.replace(/(<iframe[^>]*src=")([^"]*)(")/i, `$1${url}$3`);
        fs.writeFileSync(nfcViewPath, content);
        res.json({ success: true });
      } else {
        res.status(404).json({ error: "nfc-view.html not found" });
      }
    } catch (error) {
      console.error("Error updating nfc-view.html:", error);
      res.status(500).json({ error: "Failed to update nfc-view.html" });
    }
  });
