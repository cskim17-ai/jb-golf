...
const GolferQuotesWidget = () => {
  ...
  const [currentIndex, setCurrentIndex] = useState(() => Math.floor(Math.random() * INITIAL_QUOTES.length));
  ...
  }, [showOnlyKorean, filteredQuotes.length]);
    ...
    const fetchQuotes = async () => {
      try {
        ...
        if (!querySnapshot.empty) {
          ...
          setCurrentIndex(Math.floor(Math.random() * fetchedQuotes.length));
        }
      } catch (error) {
