  ...
  useEffect(() => {
    const fetchRate = async () => {
      try {
        let rateValue = null;
        try {
          const response = await fetch('https://cdn.jsdelivr.net/npm/@fawazahmed0/currency-api@latest/v1/currencies/myr.json');
          if (response.ok) {
            const data = await response.json();
            if (data && data.myr && data.myr.krw) {
              rateValue = data.myr.krw;
            }
          }
        } catch (e) {
        ...
        if (!rateValue) {
          ...
          if (response.ok) {
            ...
            if (data && data.rates && data.rates.KRW) {
              rateValue = data.rates.KRW;
            }
          }
        }

        if (rateValue) {
          setRate(Math.round(rateValue));
        }
      } catch (error) {
...
export const PricingContext = createContext<CoursePricing[]>([]);
export const usePricing = () => useContext(PricingContext);

export const NoticesContext = createContext<any[]>([]);
export const useNotices = () => useContext(NoticesContext);

export const GolferQuotesContext = createContext<GolferQuote[]>([]);
export const useGolferQuotes = () => useContext(GolferQuotesContext);

export const GalleryContext = createContext<{
  topics: GalleryTopic[], 
  setTopics: React.Dispatch<React.SetStateAction<GalleryTopic[]>>,
  fetchPhotos: (topicId: string, reset?: boolean) => Promise<void>
}>({topics: [], setTopics: () => {}, fetchPhotos: async () => {}});
export const useGallery = () => useContext(GalleryContext);

export const VideoGalleryContext = createContext<{topics: VideoTopic[], setTopics: React.Dispatch<React.SetStateAction<VideoTopic[]>>}>({topics: [], setTopics: () => {}});
export const useVideoGallery = () => useContext(VideoGalleryContext);
...
export const MenuConfigProvider = ({ children }: { children: React.ReactNode }) => {
  const [menuItems, setMenuItems] = useState<MenuItem[]>([]);

  useEffect(() => {
    const unsubscribe = onSnapshot(doc(db, 'settings', 'clientMenuConfig'), (docSnap) => {
      if (docSnap.exists()) {
  ...
  return (
    <MenuConfigContext.Provider value={menuItems}>
      {children}
...
export const NoticesProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  return <NoticesContext.Provider value={notices}>{children}</NoticesContext.Provider>;
};

export const GolferQuotesProvider = ({ children }: { children: React.ReactNode }) => {
  const [quotes, setQuotes] = useState<GolferQuote[]>([]);

  useEffect(() => {
    const fetchQuotes = async () => {
      try {
        const q = query(collection(db, 'golferQuotes'), orderBy('name'));
        const snapshot = await getDocs(q);
        setQuotes(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as GolferQuote)));
      } catch (error) {
        console.error("Error fetching golfer quotes:", error);
      }
    };
    fetchQuotes();
  }, []);

  return <GolferQuotesContext.Provider value={quotes}>{children}</GolferQuotesContext.Provider>;
};
...
export const GalleryProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  useEffect(() => {
    const fetchGallery = async () => {
      try {
        const topicsSnapshot = await getDocs(collection(db, 'gallery'));
  ...
  const fetchPhotos = async (topicId: string, reset: boolean = false) => {
    const topic = topics.find(t => t.id === topicId);
    if (!topic || (!reset && !topic.hasMore)) return;

    setTopics(prev => prev.map(t => t.id === topicId ? { ...t, isLoadingPhotos: true } : t));

    try {
      const q = query(
        collection(db, 'gallery', topicId, 'photos'),
        orderBy('createdAt', 'desc'),
        limit(20),
        ...(reset ? [] : [startAfter(topic.lastVisible || null)])
      );
      
      const snapshot = await getDocs(q);
      const newPhotos = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as GalleryPhoto));
      
      setTopics(prev => prev.map(t => {
        if (t.id === topicId) {
          return {
            ...t,
            photos: reset ? newPhotos : [...t.photos, ...newPhotos],
      ...
      setTopics(prev => prev.map(t => t.id === topicId ? { ...t, isLoadingPhotos: false } : t));
    }
  };

  return <GalleryContext.Provider value={{topics, setTopics, fetchPhotos}}>{children}</GalleryContext.Provider>;
};

export const VideoGalleryProvider = ({ children }: { children: React.ReactNode }) => {
  const [topics, setTopics] = useState<VideoTopic[]>([]);

  useEffect(() => {
    const fetchVideoGallery = async () => {
      try {
        const topicsSnapshot = await getDocs(collection(db, 'videoGallery'));
        const videosSnapshot = await getDocs(collectionGroup(db, 'videos'));
        
        const videosMap = new Map<string, VideoItem[]>();
        videosSnapshot.docs.forEach(doc => {
          const topicId = doc.ref.parent.parent?.id;
          if (topicId) {
            if (!videosMap.has(topicId)) videosMap.set(topicId, []);
            videosMap.get(topicId)?.push({ id: doc.id, ...doc.data() } as VideoItem);
          }
        });
  ...
  return <VideoGalleryContext.Provider value={{topics, setTopics}}>{children}</VideoGalleryContext.Provider>;
};
    ...
    <footer className="bg-ink text-paper py-10 px-6">
      <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
        <div>
          ...
          <p className="text-paper/80 text-xs leading-relaxed max-w-xs">
            조호바루 골프 여행의 품격 있는 시작. 
            최고의 숙소와 골프장을 연결하는 프리미엄 커넥트 서비스입니다.
          </p>
        </div>
        <div>
          <h3 className="text-xs tracking-widest uppercase mb-4 opacity-60">연락하기</h3>
          <p className="text-base serif">jco119@gmail.com</p>
          <p className="text-base serif">cskim1747@gmail.com</p>
          <p className="text-[10px] opacity-80 mt-2">궁금하신 사항은 위 메일로 문의해 주세요</p>
          <p className="text-xs opacity-80 mt-2">야나골 골프클럽</p>
        </div>
        <div>
      ...
      <div className="max-w-5xl mx-auto mt-10 pt-6 border-t border-paper/10 flex justify-between items-center text-[10px] tracking-widest opacity-60">
    ...
    <div className="max-w-7xl w-full relative group">
      ...
      <AnimatePresence mode="wait">
        <motion.div 
          ...
          className="grid grid-cols-1 md:grid-cols-12 gap-4 p-4 glass rounded-[40px] border border-white/10 min-h-[500px]"
          onMouseEnter={() => setIsAutoPlaying(false)}
          onMouseLeave={() => setIsAutoPlaying(true)}
        >
          {/* Left Card: 프리미엄 가이드 */}
          <div className="md:col-span-3 bg-forest/40 backdrop-blur-md rounded-[32px] px-4 py-8 border border-white/10 flex flex-col">
          ...
          <div className="md:col-span-6 relative rounded-[32px] overflow-hidden min-h-[500px]">
            <img 
          ...
          {(() => {
            ...
            return (
                <div className="md:col-span-3 bg-forest/40 backdrop-blur-md rounded-[32px] p-4 border border-white/10 flex flex-col gap-4">
                {/* Course Info */}
                <div className="bg-white/5 rounded-2xl p-4 border border-white/5">
                  ...
                  <p className="text-sm opacity-80 font-light whitespace-pre-wrap">{courseInfoSubText}</p>
                </div>
                ...
                <div className="bg-white/5 rounded-2xl p-4 border border-white/5 flex-grow">
                  ...
                  <div className="mt-4 pt-4 border-t border-white/5">
                    ...
                    <p className="text-xs opacity-80 whitespace-pre-wrap">{coursePricing.caddyInfo || (staticCourse ? `RM ${staticCourse.pricing.caddyFee}` : '')}</p>
                  </div>
                </div>
                ...
                <div className="bg-lime/10 rounded-2xl p-4 border border-lime/20">
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-sm text-lime font-bold leading-relaxed whitespace-pre-wrap">
                      {promotionSubText}
                    </p>
                  </div>
                </div>
  ...
  useEffect(() => {
    if (isQuoteModalOpen) {
      const coursesInfo = selectedCourses.map(id => {
        ...
        const scheduleDetails = (schedules || []).map(opt => {
          const dateStr = (opt.dates || []).length > 0 
            ? opt.dates.map(d => safeFormat(d, 'MM/dd')).join(', ')
            : '날짜 미선택';
      ...
      if (allDates.length > 0) {
        ...
        const start = safeFormat(sorted[0], 'yyyy.MM.dd');
        const end = safeFormat(sorted[sorted.length - 1], 'yyyy.MM.dd');
        period = `${start} ~ ${end}`;
      }
  ...
  const sendEmail = async (e: React.FormEvent) => {
    ...
    const summary = selectedCourses.map(id => {
      ...
      return schedules.map(opt => {
        ...
        const dateStr = (opt.dates || []).length > 0 ? opt.dates.map(d => safeFormat(d, 'MM/dd')).join(', ') : '날짜 미선택';
                              ...
                              <div className="flex flex-wrap items-center gap-4">
                                ...
                                <div className="relative">
                                  ...
                                  >
                                    ...
                                    <span>
                                      {opt.dates?.length > 0 
                                        ? `${safeFormat(opt.dates[0], 'MM/dd')}${opt.dates.length > 1 ? ` 외 ${opt.dates.length - 1}` : ''}`
                                        : '날짜 선택'}
                                    </span>
                                  </button>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs opacity-40 mt-1">
                      {safeFormat(notice.createdAt, 'yyyy.MM.dd')}
                    </p>
                  </div>
                </div>
...
const Admin = () => {
  ...
  const golferQuotesData = useGolferQuotes();
  const { topics: videoGalleryData, setTopics: setVideoGalleryData } = useVideoGallery();
  const [videoTopicFilter, setVideoTopicFilter] = useState<string>('all');
  const { topics: galleryData, setTopics: setGalleryData, fetchPhotos } = useGallery();
  const [galleryTopicFilter, setGalleryTopicFilter] = useState<string>('all');
  
  useEffect(() => {
    if (galleryData.length > 0 && galleryTopicFilter === 'all') {
      setGalleryTopicFilter(galleryData[0].id);
    }
    if (videoGalleryData.length > 0 && videoTopicFilter === 'all') {
      setVideoTopicFilter(videoGalleryData[0].id);
    }
  }, [galleryData, galleryTopicFilter, videoGalleryData, videoTopicFilter]);
  ...
  const [isSrchViewModalOpen, setIsSrchViewModalOpen] = useState(false);
  const [isNfcConfirmModalOpen, setIsNfcConfirmModalOpen] = useState(false);
  ...
  const moveItem = async (type: 'photo' | 'video' | 'photoTopic' | 'videoTopic', topicId: string, itemId: string, direction: 'up' | 'down') => {
    if (type === 'photoTopic' || type === 'videoTopic') {
      const collectionName = type === 'photoTopic' ? 'gallery' : 'videoGallery';
      const data = type === 'photoTopic' ? [...galleryData] : [...videoGalleryData];
      const index = data.findIndex(t => t.id === topicId);
      if (index === -1) return;
      ...
      setIsSaving(true);
      try {
        // Update all topics with their new order
        const batch = data.map((topic, idx) => 
          updateDoc(doc(db, collectionName, topic.id), { order: idx })
        );
        await Promise.all(batch);
        showAlert('주제 순서가 변경되었습니다.');
      } catch (error) {
        console.error(error);
        showAlert('주제 순서 변경에 실패했습니다.');
      } finally {
        setIsSaving(false);
      }
      return;
    ...
    const topicCollection = type === 'photo' ? 'gallery' : 'videoGallery';
    const subCollection = type === 'photo' ? 'photos' : 'videos';
    const data = type === 'photo' ? galleryData : videoGalleryData;
    
    const topic = data.find(t => t.id === topicId);
    if (!topic) return;
    
    const items = [...(type === 'photo' ? (topic as GalleryTopic).photos : (topic as VideoTopic).videos)];
    ...
    setIsSaving(true);
    try {
      // Update all items in this topic with their new order
      const batch = items.map((item, idx) => 
        updateDoc(doc(db, topicCollection, topicId, subCollection, item.id), { order: idx })
      );
      await Promise.all(batch);
  ...
  const [pendingVideos, setPendingVideos] = useState<Record<string, Partial<VideoItem>>>({});
  const [pendingRows, setPendingRows] = useState<Record<string, string[]>>({});
  const [pendingVideoRows, setPendingVideoRows] = useState<Record<string, string[]>>({});
  ...
  const fetchData = useCallback(() => {
    ...
    // Removed onSnapshot for notices

    let topics: GalleryTopic[] = [];
    let photos: GalleryPhoto[] = [];
    let videoTopics: VideoTopic[] = [];
    let videos: VideoItem[] = [];

    const updateGalleryData = () => {
      const grouped = topics.map(topic => {
        const topicPhotos = photos.filter(p => (p as any).topicId === topic.id);
        topicPhotos.sort((a, b) => (a.order ?? 999) - (b.order ?? 999) || (a.createdAt?.toMillis?.() || 0) - (b.createdAt?.toMillis?.() || 0));
        return {
          ...topic,
          photos: topicPhotos
        };
    ...
    const updateVideoGalleryData = () => {
      const grouped = videoTopics.map(topic => {
        const topicVideos = videos.filter(v => (v as any).topicId === topic.id);
        topicVideos.sort((a, b) => (a.order ?? 999) - (b.order ?? 999) || (a.createdAt?.toMillis?.() || 0) - (b.createdAt?.toMillis?.() || 0));
        return {
          ...topic,
          videos: topicVideos
        };
      });
      setVideoGalleryData(grouped);
    };

    // Removed unsubscribeGallery

    // Removed unsubscribePhotos

    // Removed unsubscribeVideoGallery

    // Removed unsubscribeVideos

    const unsubscribeChatting = onSnapshot(collection(db, 'chatting'), (snapshot) => {
      const data = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }) as Chatting);
      setChattingData(data);
    }, (error) => {
      console.error("Admin chatting fetch error:", error);
    });
  ...
  return (
    ...
    <div className="pt-40 pb-24 px-32 max-w-[1600px] mx-auto">
      {user && (
        <div className="bg-forest/60 backdrop-blur-xl p-10 rounded-[40px] border border-white/10 mb-12">
          <div className="flex flex-col gap-8">
              ...
              <div className="flex flex-wrap gap-4">
                ...
                {activeTab === 'pricing' && (
                  <>
                    <button 
                      onClick={addCourse} 
                      className="bg-white/5 hover:bg-white/10 border border-white/10 px-6 py-3 rounded-full flex items-center gap-2 transition-all text-sm"
                    >
                      <Plus size={18} /> 골프장 추가
                    </button>
                    <button 
                      ...
                      disabled={isSaving}
                      className={cn(
                        "bg-lime text-forest px-8 py-3 rounded-full font-bold flex items-center gap-2 transition-all text-sm",
            ...
            {/* Bottom Row: Search Bar (only for pricing) */}
            {activeTab === 'pricing' && (
              <div className="flex items-center gap-6">
                <div className="relative flex-grow max-w-md">
                  <select 
                    ...
                    className="bg-white/5 border border-lime/20 rounded-2xl px-6 py-4 w-full focus:border-lime outline-none transition-all appearance-none text-white text-sm pr-12"
                  >
                ...
                <span className="text-sm opacity-40 italic">총 {pricingData.length}개 등록됨</span>
              </div>
            )}
          </div>
        </div>
      ...
      {activeTab === 'pricing' && !user && !isPasswordVerified && (
      ...
      {activeTab === 'pricing' && user && (
        <div className="space-y-12">
          {pricingData.length === 0 ? (
            ...
            <div key={course.id} id={`course-${course.id}`} className="glass p-10 rounded-[40px] border border-white/10 overflow-hidden relative group transition-all duration-500">
              ...
              <tbody className="text-sm">
                {noticesData.map((notice) => (
                  <tr key={notice.id} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors">
                    ...
                    <td className="p-6 opacity-60">{safeFormat(notice.createdAt, 'yyyy.MM.dd')}</td>
      ...
      {activeTab === 'quotes' && user && (
        <div className="space-y-8">
          {/* Filters */}
          <div className="glass p-10 rounded-3xl border border-white/10 flex flex-wrap gap-6 items-end">
            <div className="flex-grow min-w-[200px]">
                  ...
                  {filteredQuotes.length === 0 ? (
                    ...
                    filteredQuotes.map((quote) => (
                      <tr key={quote.id} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors">
                        ...
                        <td className="px-6 py-4 opacity-60 whitespace-nowrap max-w-[200px] truncate" title={quote.travel_period || (quote as any).schedule?.split(' (')[0] || '-'}>
                          {quote.travel_period || (quote as any).schedule?.split(' (')[0] || '-'}
                        </td>
                        <td className="px-6 py-4 text-right font-medium whitespace-nowrap">{quote.total_myr}</td>
                        <td className="px-6 py-4 text-right text-lime font-medium whitespace-nowrap">{quote.total_krw}</td>
      ...
      {activeTab === 'videoGallery' && user && (
        <div className="space-y-8">
            ...
            <button 
              onClick={async () => {
                const newTopic: VideoTopic = {
                  id: `vtopic_${Date.now()}`,
                  title: '새 주제',
                  order: videoGalleryData.length,
                  videos: []
                };
                try {
                  const { videos, ...topicMeta } = newTopic;
                  await setDoc(doc(db, 'videoGallery', newTopic.id), topicMeta);
                  showAlert('새 주제가 추가되었습니다.');
                } catch (error) {
                  console.error("Video topic add error:", error);
          ...
          <div className="grid grid-cols-1 gap-8">
            {videoGalleryData
              .filter(topic => videoTopicFilter === 'all' || topic.id === videoTopicFilter)
              .map((topic) => {
                const originalIdx = videoGalleryData.findIndex(t => t.id === topic.id);
                return (
                  <div key={topic.id} className="flex gap-4 items-center">
                    {/* Topic Order Controls */}
                    <div className="flex flex-col gap-2">
                      <button 
                        disabled={originalIdx === 0}
                        onClick={() => moveItem('videoTopic', topic.id, '', 'up')}
                      ...
                      <button 
                        disabled={originalIdx === videoGalleryData.length - 1}
                        onClick={() => moveItem('videoTopic', topic.id, '', 'down')}
                        className="p-2 bg-white/10 border border-white/20 rounded-xl hover:bg-lime hover:text-forest disabled:opacity-20 transition-all text-white"
                        title="아래로 이동"
                      >
                        <ChevronDown size={20} />
                    ...
                    <div className="flex-grow p-12 rounded-[40px] border-2 border-white space-y-6">
                      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                        <div className="flex-grow">
                          ...
                          <input 
                            ...
                            value={topic.title}
                            onChange={(e) => {
                              const newData = [...videoGalleryData];
                              newData[originalIdx].title = e.target.value;
                              setVideoGalleryData(newData);
                            }}
                  ...
                  <div className="flex gap-4 items-center">
                    <button 
                      onClick={async () => {
                        setIsSaving(true);
                        try {
                          const { videos, ...topicMeta } = topic;
                          await setDoc(doc(db, 'videoGallery', topic.id), topicMeta);
                          showAlert('저장되었습니다.');
                        } catch (error) {
                          console.error("Video topic save error:", error);
                          showAlert('저장에 실패했습니다.');
                        } finally {
                          setIsSaving(false);
                        }
                      }}
                      className="bg-lime/20 text-lime px-6 py-3 rounded-full font-bold hover:bg-lime hover:text-forest transition-all"
                    >
                      이 주제 저장
                    </button>
                    <button 
                      onClick={() => {
                        showConfirm('이 주제와 포함된 모든 동영상을 삭제하시겠습니까?', async () => {
                          try {
                            await deleteDoc(doc(db, 'videoGallery', topic.id));
                            showAlert('삭제되었습니다.');
                          } catch (error) {
                            console.error("Video topic delete error:", error);
                            showAlert('삭제에 실패했습니다.');
                          }
                        });
                      }}
                ...
                <div className="p-6 rounded-[30px] border-2 border-white bg-transparent space-y-4">
                  {topic.videos.map((video, videoIdx) => (
                    <div key={video.id} className="p-6 rounded-2xl bg-white/5 border border-white/10 space-y-4 relative group/vitem">
                      <div className="absolute -left-4 top-1/2 -translate-y-1/2 flex flex-col gap-1 opacity-0 group-hover/vitem:opacity-100 transition-opacity">
                        <button 
                          disabled={videoIdx === 0}
                          onClick={() => moveItem('video', topic.id, video.id, 'up')}
                          className="p-1 bg-forest border border-white/20 rounded-md hover:bg-lime hover:text-forest disabled:opacity-20 disabled:hover:bg-forest disabled:hover:text-white"
                        >
                          <ChevronUp size={14} />
                        </button>
                        <button 
                          disabled={videoIdx === topic.videos.length - 1}
                          onClick={() => moveItem('video', topic.id, video.id, 'down')}
                          className="p-1 bg-forest border border-white/20 rounded-md hover:bg-lime hover:text-forest disabled:opacity-20 disabled:hover:bg-forest disabled:hover:text-white"
                        >
                          <ChevronDown size={14} />
                        </button>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-2">
                          <label className="text-[10px] tracking-widest uppercase opacity-40 font-bold">동영상 타이틀</label>
                          <input 
                            type="text"
                            value={video.title}
                            onChange={(e) => {
                              const newData = [...videoGalleryData];
                              newData[originalIdx].videos[videoIdx].title = e.target.value;
                              setVideoGalleryData(newData);
                            }}
                            className="bg-white/10 border border-white/20 rounded-xl px-4 py-2 w-full focus:border-lime outline-none transition-all text-white"
                          />
                        </div>
                        <div className="space-y-2">
                          <label className="text-[10px] tracking-widest uppercase opacity-40 font-bold">동영상 URL</label>
                          <input 
                            type="text"
                            value={video.url}
                            onChange={(e) => {
                              const newData = [...videoGalleryData];
                              newData[originalIdx].videos[videoIdx].url = e.target.value;
                              setVideoGalleryData(newData);
                            }}
                            className="bg-white/10 border border-white/20 rounded-xl px-4 py-2 w-full focus:border-lime outline-none transition-all text-white"
                          />
                      ...
                      <div className="space-y-2">
                        <label className="text-[10px] tracking-widest uppercase opacity-40 font-bold">동영상 내용 (500자 이내)</label>
                        <textarea 
                          ...
                          onChange={(e) => {
                            const newData = [...videoGalleryData];
                            newData[originalIdx].videos[videoIdx].description = e.target.value;
                            setVideoGalleryData(newData);
                          }}
                          className="bg-white/10 border border-white/20 rounded-xl px-4 py-2 w-full focus:border-lime outline-none transition-all text-white resize-none"
                        />
                      </div>
                      <div className="flex justify-end gap-2">
                        <button 
                          onClick={async () => {
                            setIsSaving(true);
                            try {
                              await updateDoc(doc(db, 'videoGallery', topic.id, 'videos', video.id), {
                                title: video.title,
                                url: video.url,
                          ...
                          className="bg-lime/20 text-lime px-4 py-2 rounded-xl transition-all text-sm font-bold hover:bg-lime hover:text-forest"
                        >
                          저장
                        </button>
                        <button 
                          onClick={() => {
                            showConfirm('이 동영상을 삭제하시겠습니까?', async () => {
                              setIsSaving(true);
                              try {
                                await deleteDoc(doc(db, 'videoGallery', topic.id, 'videos', video.id));
                          ...
                          className="bg-red-500/10 text-red-400 px-4 py-2 rounded-xl transition-all text-sm font-bold hover:bg-red-500 hover:text-white"
                        >
                          삭제
                        </button>
                      </div>
                  ...
                  {pendingVideoRows[topic.id]?.map(pendingId => (
                    <div key={pendingId} className="p-6 rounded-2xl bg-white/10 border-2 border-lime/30 space-y-4">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-2">
                          <label className="text-[10px] tracking-widest uppercase opacity-40 font-bold">동영상 타이틀</label>
                          <input 
                            type="text"
                            placeholder="타이틀 입력"
                            value={pendingVideos[`${topic.id}_${pendingId}`]?.title || ''}
                            onChange={(e) => setPendingVideos(prev => ({ 
                              ...prev, 
                              [`${topic.id}_${pendingId}`]: { ...prev[`${topic.id}_${pendingId}`], title: e.target.value } 
                            }))}
                            className="bg-white/10 border border-white/20 rounded-xl px-4 py-2 w-full focus:border-lime outline-none transition-all text-white"
                          />
                        </div>
                        <div className="space-y-2">
                          ...
                          <input 
                            ...
                            value={pendingVideos[`${topic.id}_${pendingId}`]?.url || ''}
                            onChange={(e) => setPendingVideos(prev => ({ 
                              ...prev, 
                              [`${topic.id}_${pendingId}`]: { ...prev[`${topic.id}_${pendingId}`], url: e.target.value } 
                            }))}
                            className="bg-white/10 border border-white/20 rounded-xl px-4 py-2 w-full focus:border-lime outline-none transition-all text-white"
                          />
                        </div>
                      ...
                      <div className="space-y-2">
                        <label className="text-[10px] tracking-widest uppercase opacity-40 font-bold">동영상 내용 (500자 이내)</label>
                        <textarea 
                          placeholder="동영상 설명 입력"
                          value={pendingVideos[`${topic.id}_${pendingId}`]?.description || ''}
                          maxLength={500}
                          rows={3}
                          onChange={(e) => setPendingVideos(prev => ({ 
                            ...prev, 
                            [`${topic.id}_${pendingId}`]: { ...prev[`${topic.id}_${pendingId}`], description: e.target.value } 
                          }))}
                          className="bg-white/10 border border-white/20 rounded-xl px-4 py-2 w-full focus:border-lime outline-none transition-all text-white resize-none"
                        />
                      </div>
                      <div className="flex justify-end gap-2">
                        <button 
                          onClick={async () => {
                            const video = pendingVideos[`${topic.id}_${pendingId}`];
                            if (!video?.title || !video?.url) {
                              showAlert('타이틀과 URL을 입력해주세요.');
                              return;
                            }
                            setIsSaving(true);
                            try {
                              const videoId = `video_${Date.now()}`;
                              await setDoc(doc(db, 'videoGallery', topic.id, 'videos', videoId), {
                                title: video.title,
                                url: video.url,
                                description: video.description || '',
                                createdAt: serverTimestamp()
                              });
                              setPendingVideoRows(prev => ({
                                ...prev,
                                [topic.id]: prev[topic.id].filter(id => id !== pendingId)
                              }));
                              setPendingVideos(prev => {
                                const next = { ...prev };
                                delete next[`${topic.id}_${pendingId}`];
                                return next;
                              });
                          ...
                          className="bg-lime text-forest px-6 py-2 rounded-xl transition-all text-sm font-bold"
                        >
                          추가하기
                        </button>
                        <button 
                          ...
                          className="bg-white/10 text-white/60 px-6 py-2 rounded-xl transition-all text-sm font-bold"
                        >
                          취소
                        </button>
                      </div>
                  ...
                  <button 
                    onClick={() => {
                      setPendingVideoRows(prev => ({
                        ...prev,
                        [topic.id]: [...(prev[topic.id] || []), `vpending_${Date.now()}`]
                      }));
                    }}
                    className="text-lime text-sm font-bold flex items-center gap-2 mt-4 hover:opacity-80 transition-opacity px-2"
                  >
                    <Plus size={16} /> 동영상 추가
                  </button>
                </div>
      ...
      {activeTab === 'gallery' && user && (
        <div className="space-y-8">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            ...
            <button 
              onClick={async () => {
                ...
                try {
                  const { photos, ...topicMeta } = newTopic;
                  await setDoc(doc(db, 'gallery', newTopic.id), topicMeta);
                  showAlert('새 주제가 추가되었습니다.');
                } catch (error) {
                  console.error("Gallery topic add error:", error);
                  showAlert('추가에 실패했습니다.');
                }
              }}
              className="bg-lime text-forest px-6 py-3 rounded-full font-bold flex items-center gap-2 hover:shadow-[0_0_30px_rgba(163,230,53,0.3)] transition-all"
          ...
          <div className="grid grid-cols-1 gap-8">
            {galleryData
              .filter(topic => galleryTopicFilter === 'all' || topic.id === galleryTopicFilter)
              .map((topic) => {
                const originalIdx = galleryData.findIndex(t => t.id === topic.id);
                ...
                return (
                  <div key={topic.id} className="flex gap-4 items-center">
                    ...
                    <div className="flex flex-col gap-2">
                      <button 
                        disabled={originalIdx === 0}
                        onClick={() => moveItem('photoTopic', topic.id, '', 'up')}
                        className="p-2 bg-white/10 border border-white/20 rounded-xl hover:bg-lime hover:text-forest disabled:opacity-20 transition-all text-white"
                        title="위로 이동"
                      >
                        <ChevronUp size={20} />
                      ...
                      <button 
                        disabled={originalIdx === galleryData.length - 1}
                        onClick={() => moveItem('photoTopic', topic.id, '', 'down')}
                        className="p-2 bg-white/10 border border-white/20 rounded-xl hover:bg-lime hover:text-forest disabled:opacity-20 transition-all text-white"
                        title="아래로 이동"
                      >
                        <ChevronDown size={20} />
                      ...
                      <button 
                        onClick={() => {
                          ...
                          if (isExpanded) {
                            ...
                            fetchPhotos(topic.id, true);
                          }
                          setExpandedTopics(newExpanded);
                        }}
                        className="bg-white/10 border border-white/20 rounded-xl px-4 py-2 hover:bg-lime hover:text-forest transition-all text-white font-bold whitespace-nowrap"
                      >
                        {isExpanded ? "사진 숨기기" : "[사진보기]"}
                      </button>
                    </div>
                    <div className="flex-grow p-12 rounded-[40px] border-2 border-white space-y-6">
                      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                          ...
                          <div className="flex gap-4 items-center">
                            <button 
                              onClick={async () => {
                                setIsSaving(true);
                                try {
                                  // Save only topic metadata
                                  const { photos, ...topicMeta } = topic;
                                  await setDoc(doc(db, 'gallery', topic.id), topicMeta);
                                  showAlert('저장되었습니다.');
                                } catch (error) {
                              ...
                              className="bg-lime text-forest px-6 py-3 rounded-full font-bold hover:shadow-[0_0_20px_rgba(163,230,53,0.4)] transition-all"
                            >
                              저장
                            </button>
                            <button 
                              onClick={() => {
                                showConfirm('이 주제와 포함된 모든 사진을 삭제하시겠습니까?', async () => {
                                  try {
                                    await deleteDoc(doc(db, 'gallery', topic.id));
                                    showAlert('삭제되었습니다.');
                                  } catch (error) {
                            ...
                            onChange={async (e) => {
                              ...
                              if (files && files.length > 0) {
                                ...
                                for (let i = 0; i < files.length; i++) {
                                  const file = files[i];
                                  // 1MB limit for bulk upload to prevent browser crash/timeout
                                    ...
                                    const promise = new Promise<void>((resolve, reject) => {
                                      reader.onloadend = async () => {
                                        try {
                                          ...
                                          await setDoc(doc(db, 'gallery', topic.id, 'photos', photoId), {
                                            url: compressed,
                                            caption: '',
                                            order: (topic.photos?.length || 0) + i,
                                            createdAt: serverTimestamp()
                                          });
                                          successCount++;
                                          resolve();
                                ...
                                setIsSaving(false);
                                showAlert(`${successCount}개의 사진이 업로드되었습니다.${failCount > 0 ? ` (${failCount}개 실패 - 용량 초과 등)` : ''}`);
                                fetchPhotos(topic.id, true); // Refresh photos
                              }
                            }}
                          />
                          <button className="bg-white/5 border border-white/10 hover:bg-white/10 text-white/80 px-6 py-2 rounded-xl text-sm font-bold transition-all flex items-center gap-2">
                            <Upload size={16} /> 일괄 사진 업로드
                          </button>
                        </div>

                <div className="p-6 rounded-[30px] border-2 border-white bg-transparent space-y-4">
                  {topic.isLoadingPhotos && (
                  ...
                  {topic.photos.map((photo, photoIdx) => (
                    <div key={photo.id} className="flex gap-4 items-center relative group/pitem">
                      <div className="absolute -left-10 top-1/2 -translate-y-1/2 flex flex-col gap-1 opacity-0 group-hover/pitem:opacity-100 transition-opacity">
                        <button 
                          disabled={photoIdx === 0}
                          onClick={() => moveItem('photo', topic.id, photo.id, 'up')}
                          className="p-1 bg-forest border border-white/20 rounded-md hover:bg-lime hover:text-forest disabled:opacity-20 disabled:hover:bg-forest disabled:hover:text-white"
                        >
                          <ChevronUp size={14} />
                        </button>
                        <button 
                          disabled={photoIdx === topic.photos.length - 1}
                          onClick={() => moveItem('photo', topic.id, photo.id, 'down')}
                          className="p-1 bg-forest border border-white/20 rounded-md hover:bg-lime hover:text-forest disabled:opacity-20 disabled:hover:bg-forest disabled:hover:text-white"
                        >
                          <ChevronDown size={14} />
                        </button>
                      ...
                      <div className="flex-grow space-y-2">
                        <input 
                          ...
                          value={photo.url} 
                          readOnly
                          className="bg-white/10 border border-white/20 rounded-xl px-4 py-2 text-xs w-full text-white/60 outline-none truncate"
                        />
                        <div className="flex gap-2 items-center">
                              ...
                              onChange={async (e) => {
                                ...
                                if (file) {
                                  if (file.size > 700 * 1024) {
                                    showAlert('파일 크기가 너무 큽니다. 700KB 이하의 이미지를 사용해주세요.');
                                    return;
                                  }
                                  setIsSaving(true);
                                  try {
                                    ...
                                    reader.onloadend = async () => {
                                      const compressed = await compressImage(reader.result as string);
                                      await updateDoc(doc(db, 'gallery', topic.id, 'photos', photo.id), {
                                        url: compressed
                                      });
                                      
                                      // Update local state for immediate preview
                                      const newData = [...galleryData];
                                      const tIdx = newData.findIndex(t => t.id === topic.id);
                                      const pIdx = newData[tIdx].photos.findIndex(p => p.id === photo.id);
                                      newData[tIdx].photos[pIdx].url = compressed;
                                      setGalleryData(newData);
                                      
                                      showAlert('사진이 업데이트되었습니다.');
                                      setIsSaving(false);
                                    };
                                    reader.readAsDataURL(file);
                                  } catch (error) {
                                    console.error(error);
                                    showAlert('업데이트에 실패했습니다.');
                                    setIsSaving(false);
                                  }
                                }
                              }}
                            />
                            <button className="bg-white/10 hover:bg-white/20 border border-white/20 px-4 py-2 rounded-xl transition-all text-sm flex items-center gap-2 text-white w-fit">
                              <ImageIcon size={16} />
                              사진 업로드
                            </button>
                          </div>
                          <button 
                            onClick={() => {
                              showConfirm('이 사진을 삭제하시겠습니까?', async () => {
                                setIsSaving(true);
                                try {
                                  await deleteDoc(doc(db, 'gallery', topic.id, 'photos', photo.id));
                            ...
                            className="bg-transparent hover:bg-red-500/20 hover:text-red-400 border border-transparent hover:border-red-500/20 px-3 py-2 rounded-xl transition-all text-sm flex items-center gap-2 text-white/40"
                          >
                            <Trash2 size={16} />
                          </button>
                          ...
                          <input 
                            ...
                            onChange={(e) => {
                              ...
                              const tIdx = newData.findIndex(t => t.id === topic.id);
                              const pIdx = newData[tIdx].photos.findIndex(p => p.id === photo.id);
                              newData[tIdx].photos[pIdx].caption = e.target.value;
                              setGalleryData(newData);
                            }}
                            placeholder="사진 설명을 입력하세요"
                            className="bg-white/5 border border-white/10 rounded-xl px-4 py-2 text-xs flex-grow focus:border-lime outline-none transition-all text-white"
                          />
                          <button 
                            onClick={async () => {
                              setIsSaving(true);
                      ...
                      <div className="w-32 h-20 rounded-lg overflow-hidden flex-shrink-0 border border-white/20">
                        <img src={photo.thumb_url || photo.url} alt="" className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                      </div>
                    </div>
                  ))}
                  ...
                  {topic.hasMore && (
                    <button 
                      onClick={() => fetchPhotos(topic.id)}
                      className="w-full py-3 bg-white/10 rounded-xl text-white font-bold hover:bg-white/20 transition-all"
                    >
                      더 보기 (Load More)
                    </button>
                  )}

                  {pendingRows[topic.id]?.map(pendingId => (
                    <div key={pendingId} className="flex gap-4 items-center bg-white/5 p-4 rounded-xl border border-white/10">
                      <div className="flex-grow space-y-2">
                        <input 
                          type="text" 
                          placeholder="이미지 URL을 입력하거나 파일을 업로드하세요"
                          className="bg-white/5 border border-white/10 rounded-xl px-4 py-2 text-xs w-full focus:border-lime outline-none"
                          value={pendingPhotos[`${topic.id}_${pendingId}`] || ''}
                          onChange={(e) => setPendingPhotos(prev => ({ ...prev, [`${topic.id}_${pendingId}`]: e.target.value }))}
                        />
                        <input 
                          type="text" 
                          placeholder="사진 설명을 입력하세요"
                          className="bg-white/5 border border-white/10 rounded-xl px-4 py-2 text-xs w-full focus:border-lime outline-none"
                          value={pendingCaptions[`${topic.id}_${pendingId}`] || ''}
                          onChange={(e) => setPendingCaptions(prev => ({ ...prev, [`${topic.id}_${pendingId}`]: e.target.value }))}
                        />
                        <div className="flex gap-2">
                          <div className="relative">
                                ...
                                if (file) {
                                  ...
                                  try {
                                    ...
                                    reader.onloadend = async () => {
                                      ...
                                      await setDoc(doc(db, 'gallery', topic.id, 'photos', photoId), {
                                        url: compressed,
                                        caption: pendingCaptions[`${topic.id}_${pendingId}`] || '',
                                        createdAt: serverTimestamp()
                                      });
                            ...
                            />
                            <button className="bg-white/5 hover:bg-white/10 border border-white/10 px-4 py-2 rounded-xl transition-all text-sm flex items-center gap-2">
                              <ImageIcon size={16} />
                              사진 업로드
                            </button>
                          </div>
                          {pendingPhotos[`${topic.id}_${pendingId}`] && (
                            <button 
                              onClick={async () => {
                                setIsSaving(true);
                                try {
                                  const photoId = `photo_${Date.now()}`;
                                  await setDoc(doc(db, 'gallery', topic.id, 'photos', photoId), {
                                    url: pendingPhotos[`${topic.id}_${pendingId}`],
                                    caption: pendingCaptions[`${topic.id}_${pendingId}`] || '',
                                    createdAt: serverTimestamp()
                                  });
                                  setPendingRows(prev => ({
                              ...
                              className="bg-lime text-forest px-4 py-2 rounded-xl transition-all text-sm font-bold"
                            >
                              URL 저장
                            </button>
                          )}
                          <button 
                            ...
                            className="bg-white/5 hover:bg-white/10 text-white/60 border border-white/10 px-4 py-2 rounded-xl transition-all text-sm flex items-center gap-2"
                          >
                            취소
                          </button>
                        </div>
                      </div>
                      <div className="w-32 h-20 rounded-lg overflow-hidden flex-shrink-0 border border-white/10 bg-black/20 flex items-center justify-center text-white/20">
                        {pendingPhotos[`${topic.id}_${pendingId}`] ? (
                          <img 
                            src={pendingPhotos[`${topic.id}_${pendingId}`]} 
                            alt="Preview" 
                            className="w-full h-full object-cover" 
                            referrerPolicy="no-referrer"
                            onError={(e) => {
                              (e.target as HTMLImageElement).src = '';
                              (e.target as HTMLImageElement).parentElement!.innerHTML = '<span class="text-[10px]">Invalid URL</span>';
                            }}
                          />
                        ) : (
                          <ImageIcon size={24} />
                  ...
                  <button 
                    onClick={() => {
                      setPendingRows(prev => ({
                        ...prev,
                        [topic.id]: [...(prev[topic.id] || []), `pending_${Date.now()}`]
                      }));
                    }}
                    className="text-lime text-sm font-bold flex items-center gap-2 mt-4 hover:opacity-80 transition-opacity px-2"
                  >
          ...
          <div className="glass p-12 rounded-[40px] border border-white/10 space-y-8">
            ...
            {selectedChatId && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-in fade-in slide-in-from-top-4 duration-500">
                <div className="space-y-6">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input 
                        ...
                        onChange={(e) => {
                          setChattingData(chattingData.map(c => c.id === selectedChatId ? { ...c, name_ko: e.target.value } : c));
                        }}
                    ...
                    <div>
                      ...
                      <input 
                        ...
                        onChange={(e) => {
                          setChattingData(chattingData.map(c => c.id === selectedChatId ? { ...c, name_en: e.target.value } : c));
                        }}
                        className="bg-white/5 border border-white/10 rounded-xl px-4 py-2 w-full focus:border-lime outline-none transition-all"
                      />
                    </div>
                  ...
                  <div>
                    ...
                    <select 
                      ...
                      onChange={(e) => {
                        setChattingData(chattingData.map(c => c.id === selectedChatId ? { ...c, region_tag: e.target.value } : c));
                      }}
                      className="bg-white/5 border border-white/10 rounded-xl px-4 py-2 w-full focus:border-lime outline-none transition-all appearance-none cursor-pointer"
                    >
                      <option value="[시내권]" className="bg-forest">[시내권]</option>
                      <option value="[이스칸다르]" className="bg-forest">[이스칸다르]</option>
                      <option value="[세나이]" className="bg-forest">[세나이]</option>
                    </select>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="text-[10px] tracking-widest uppercase opacity-40 block mb-2 ml-2 font-bold">평일 오전 가격 (MYR) price_wd_am</label>
                      <input 
                        type="number"
                        value={chattingData.find(c => c.id === selectedChatId)?.price_wd_am || 0}
                        onChange={(e) => {
                          setChattingData(chattingData.map(c => c.id === selectedChatId ? { ...c, price_wd_am: Number(e.target.value) } : c));
                        }}
                        className="bg-white/5 border border-white/10 rounded-xl px-4 py-2 w-full focus:border-lime outline-none transition-all"
                      />
                    </div>
                    <div>
                      <label className="text-[10px] tracking-widest uppercase opacity-40 block mb-2 ml-2 font-bold">총 홀수 total_holes</label>
                      <input 
                        type="number"
                        value={chattingData.find(c => c.id === selectedChatId)?.total_holes || 0}
                        onChange={(e) => {
                          setChattingData(chattingData.map(c => c.id === selectedChatId ? { ...c, total_holes: Number(e.target.value) } : c));
                        }}
                        className="bg-white/5 border border-white/10 rounded-xl px-4 py-2 w-full focus:border-lime outline-none transition-all"
                      />
                    </div>
                  ...
                  <div>
                    <label className="text-[10px] tracking-widest uppercase opacity-40 block mb-2 ml-2 font-bold">가격 비고 (채팅 답변용) price_note</label>
                    <input 
                      type="text"
                      value={chattingData.find(c => c.id === selectedChatId)?.price_note || ''}
                      onChange={(e) => {
                        setChattingData(chattingData.map(c => c.id === selectedChatId ? { ...c, price_note: e.target.value } : c));
                      }}
                      className="bg-white/5 border border-white/10 rounded-xl px-4 py-2 w-full focus:border-lime outline-none transition-all"
                      placeholder="예: 평일 315 MYR / 주말 450 MYR"
                    />
                  </div>
                  <div>
                    <label className="text-[10px] tracking-widest uppercase opacity-40 block mb-2 ml-2 font-bold">코스 이름 (채팅 답변용) course_names</label>
                    <input 
                      type="text"
                      value={chattingData.find(c => c.id === selectedChatId)?.course_names || ''}
                      onChange={(e) => {
                        setChattingData(chattingData.map(c => c.id === selectedChatId ? { ...c, course_names: e.target.value } : c));
                      }}
                      className="bg-white/5 border border-white/10 rounded-xl px-4 py-2 w-full focus:border-lime outline-none transition-all"
                      placeholder="예: 레거시 코스, 클래식 코스"
                    />
                  </div>
                  <div>
                    <label className="text-[10px] tracking-widest uppercase opacity-40 block mb-2 ml-2 font-bold">코스 상세 설명 (채팅 답변용) course_desc_legacy</label>
                    <textarea 
                      value={chattingData.find(c => c.id === selectedChatId)?.course_desc_legacy || ''}
                      onChange={(e) => {
                        setChattingData(chattingData.map(c => c.id === selectedChatId ? { ...c, course_desc_legacy: e.target.value } : c));
                      }}
                      className="bg-white/5 border border-white/10 rounded-xl px-4 py-2 w-full focus:border-lime outline-none transition-all min-h-[60px]"
                    />
                  </div>
                  <div>
                    ...
                    <input 
                      ...
                      onChange={(e) => {
                        setChattingData(chattingData.map(c => c.id === selectedChatId ? { ...c, image_url_main: e.target.value } : c));
                      }}
                      className="bg-white/5 border border-white/10 rounded-xl px-4 py-2 w-full focus:border-lime outline-none transition-all"
                    />
                  </div>
                  <div>
                    <label className="text-[10px] tracking-widest uppercase opacity-40 block mb-2 ml-2 font-bold">검색 키워드 (쉼표로 구분) search_keywords</label>
                    <input 
                      type="text"
                      value={chattingData.find(c => c.id === selectedChatId)?.search_keywords || ''}
                      onChange={(e) => {
                        setChattingData(chattingData.map(c => c.id === selectedChatId ? { ...c, search_keywords: e.target.value } : c));
                      }}
                      className="bg-white/5 border border-white/10 rounded-xl px-4 py-2 w-full focus:border-lime outline-none transition-all"
                      placeholder="예: 포레스트, 시티, 조호바루"
                    />
                  </div>
                </div>
                ...
                <div className="space-y-6">
                  <div>
                    <label className="text-[10px] tracking-widest uppercase opacity-40 block mb-2 ml-2 font-bold">상세 위치 location</label>
                    <input 
                      type="text"
                      value={chattingData.find(c => c.id === selectedChatId)?.location || ''}
                      onChange={(e) => {
                        setChattingData(chattingData.map(c => c.id === selectedChatId ? { ...c, location: e.target.value } : c));
                      }}
                      className="bg-white/5 border border-white/10 rounded-xl px-4 py-2 w-full focus:border-lime outline-none transition-all"
                    />
                  </div>
                  <div>
                    ...
                    <textarea 
                      ...
                      onChange={(e) => {
                        setChattingData(chattingData.map(c => c.id === selectedChatId ? { ...c, main_features: e.target.value } : c));
                      }}
                  ...
                  <div>
                    ...
                    <textarea 
                      ...
                      onChange={(e) => {
                        setChattingData(chattingData.map(c => c.id === selectedChatId ? { ...c, expert_pros: e.target.value } : c));
                      }}
                      className="bg-white/5 border border-white/10 rounded-xl px-4 py-2 w-full focus:border-lime outline-none transition-all min-h-[80px]"
                    />
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input 
                        ...
                        onChange={(e) => {
                          setChattingData(chattingData.map(c => c.id === selectedChatId ? { ...c, contact_no: e.target.value } : c));
                        }}
                        className="bg-white/5 border border-white/10 rounded-xl px-4 py-2 w-full focus:border-lime outline-none transition-all"
                      />
                    </div>
                    <div>
                      ...
                      <input 
                        ...
                        onChange={(e) => {
                          setChattingData(chattingData.map(c => c.id === selectedChatId ? { ...c, map_url: e.target.value } : c));
                        }}
                        className="bg-white/5 border border-white/10 rounded-xl px-4 py-2 w-full focus:border-lime outline-none transition-all"
                      />
                    </div>
                  ...
                  <div className="pt-4 flex gap-4">
                    <button 
                      ...
                      className="flex-grow bg-lime text-forest py-4 rounded-2xl font-bold hover:shadow-[0_0_30px_rgba(163,230,53,0.3)] transition-all"
                    >
                      채팅 정보 저장하기
                    </button>
                    <button 
        ...
        <div className="space-y-8">
          ...
          <div className="glass p-8 rounded-[40px] border border-white/10 space-y-8">
              ...
              <div className="flex gap-2 w-full md:w-auto">
                ...
                <button 
                  onClick={async () => {
                    ...
                    try {
                      // 1. Update Firestore desk_1
                      await setDoc(doc(db, 'active_sessions', 'desk_1'), {
                        desk_id: 1,
            ...
            <div className="flex flex-col md:flex-row items-start md:items-center gap-4">
              ...
              <div className="bg-white/5 border border-white/10 rounded-xl px-4 py-3 flex-grow w-full min-h-[50px] break-all">
                {labOutputUrl}
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-4">
              {/* URL Result */}
              <div className="space-y-4">
                <h3 className="text-center text-lg font-bold">URL 실행결과</h3>
              ...
              {/* QR Result */}
              <div className="space-y-4">
                <h3 className="text-center text-lg font-bold">QR전송 실행결과</h3>
                  ...
                  {labQrResult ? (
                    <div className="text-center">
                      <div className="w-48 h-48 bg-black/5 rounded-xl mb-4 flex items-center justify-center mx-auto">
                        <QRCodeSVG value={labOutputUrl} size={160} />
                      </div>
              ...
              {/* NFC Result */}
              <div className="space-y-4">
                <h3 className="text-center text-lg font-bold">NFC전송 실행결과</h3>
                <div className="border border-white/20 rounded-xl overflow-hidden bg-white aspect-[3/4] relative p-4 flex flex-col items-center justify-center">
                  {labTransferResult ? (
                    <div className="text-center">
                      <div className="w-24 h-24 bg-lime/20 text-forest rounded-full mb-4 flex items-center justify-center mx-auto">
                        <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M6 2L3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4z"></path><line x1="3" y1="6" x2="21" y2="6"></line><path d="M16 10a4 4 0 0 1-8 0"></path></svg>
                      </div>
        ...
        {selectedQuote && (
          ...
          >
            ...
            >
                  ...
                  <div className="glass p-6 rounded-3xl border border-white/5 space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-base">{safeFormat(selectedQuote.timestamp, 'yyyy-MM-dd HH:mm')}</p>
                      </div>
                      <div>
              ...
              {/* Footer */}
              <div className="p-6 border-t border-white/10 text-center shrink-0 flex flex-wrap justify-center gap-4">
                <button 
                  onClick={() => setShowQR(true)}
                  className="bg-white/10 text-white px-6 py-3 rounded-full font-bold hover:bg-white/20 transition-all"
                >
                  QR코드 보기
                </button>
                <button 
        ...
        {showQR && selectedQuote && (
          ...
          >
            <motion.div 
              ...
              onClick={(e) => e.stopPropagation()}
            >
              <h3 className="text-2xl font-bold text-forest mb-6">스마트폰으로 스캔하세요</h3>
              <div className="bg-white p-4 rounded-xl shadow-inner mb-6">
                <QRCodeSVG 
                  value={`${window.location.origin}/quote/${selectedQuote.id}`} 
                  size={256} 
                  level="H"
                  includeMargin={true}
                />
              </div>
              <p className="text-forest/60 text-sm mb-6 text-center max-w-xs">
                스마트폰 카메라로 위 QR 코드를 스캔하면<br/>견적서 모바일 뷰를 확인할 수 있습니다.
              </p>
              <button 
                onClick={() => setShowQR(false)}
                className="bg-forest text-white px-8 py-3 rounded-full font-bold hover:bg-forest/80 transition-all w-full"
              >
                닫기
              </button>
            </motion.div>
        ...
        {isSrchViewModalOpen && (
          ...
          >
            <motion.div 
              ...
              animate={{ scale: 1, opacity: 1, y: 0 }}
              exit={{ scale: 0.9, opacity: 0, y: 20 }}
              className="glass max-w-4xl w-full h-[80vh] rounded-[40px] border border-white/20 overflow-hidden shadow-2xl flex flex-col"
            >
              <div className="p-6 border-b border-white/10 flex justify-between items-center shrink-0">
                <h2 className="text-2xl serif text-white">srch-view.html (미리보기)</h2>
                <button 
                  onClick={() => setIsSrchViewModalOpen(false)}
                  className="p-2 hover:bg-white/10 rounded-full transition-all"
                >
                  <X size={24} />
                </button>
              ...
              <div className="flex-grow relative bg-white">
                <iframe 
                  src={labOutputUrl} 
                  className="w-full h-full border-0"
                  title="srch-view.html preview"
                />
              </div>
              <div className="p-6 border-t border-white/10 flex justify-end gap-4 shrink-0">
                <button 
                  onClick={() => setIsSrchViewModalOpen(false)}
                  className="bg-white/10 hover:bg-white/20 text-white px-8 py-3 rounded-xl font-bold border border-white/20 transition-all"
                >
                  닫기
                </button>
                <button 
                  onClick={() => {
                    setIsSrchViewModalOpen(false);
                    setIsNfcConfirmModalOpen(true);
                  }}
        ...
        {isNfcConfirmModalOpen && (
          ...
          >
            <motion.div 
              ...
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.9, opacity: 0 }}
              className="glass p-8 rounded-3xl max-w-lg w-full shadow-2xl border border-white/20"
            >
              <h3 className="text-2xl serif text-white mb-6 text-center">화면 전송 확인</h3>
              <div className="bg-white/5 border border-white/10 rounded-xl p-6 mb-8">
                <p className="text-sm text-white/60 mb-2">다음 화면(URL)을 모바일 기기로 전송합니다:</p>
                <p className="text-lime font-mono break-all">{labOutputUrl}</p>
              </div>
              <div className="flex gap-4">
                <button 
                  onClick={() => setIsNfcConfirmModalOpen(false)}
                  className="flex-1 py-4 rounded-xl border border-white/10 hover:bg-white/5 transition-all font-medium"
                >
                  취소
                </button>
                <button 
                  onClick={async () => {
                    setIsNfcConfirmModalOpen(false);
                    
                    // Firestore에 데이터 저장 (active_sessions/desk_1)
                    try {
                      await setDoc(doc(db, 'active_sessions', 'desk_1'), {
                        desk_id: 1,
                        content: `<iframe src="${labOutputUrl}" style="width:100%; height:100%; border:none;"></iframe>`,
                      ...
                      setLabTransferResult('화면 전송이 성공적으로 완료되었습니다!');
                      showAlert('DB 전송이 완료되었습니다!');
                    } catch (error) {
                      console.error("Firestore save error:", error);
                      setLabTransferResult('화면 전송 실패: ' + (error as Error).message);
                      showAlert('DB 저장 중 오류가 발생했습니다.');
                    }
                  }}
                  className="flex-1 py-4 rounded-xl bg-lime text-forest font-bold hover:shadow-[0_0_20px_rgba(163,230,53,0.3)] transition-all"
                >
                  확인
                </button>
              </div>
...
const Gallery = () => {
  ...
  useEffect(() => {
    if (galleryTopics.length > 0 && selectedPhotoTopicId === 'all') {
      setSelectedPhotoTopicId(galleryTopics[0].id);
    }
    if (videoTopics.length > 0 && selectedVideoTopicId === 'all') {
      setSelectedVideoTopicId(videoTopics[0].id);
    }
  }, [galleryTopics, selectedPhotoTopicId, videoTopics, selectedVideoTopicId]);

  useEffect(() => {
    let topics: GalleryTopic[] = [];
    let photos: GalleryPhoto[] = [];
    let vTopics: VideoTopic[] = [];
    let videos: VideoItem[] = [];
    ...
    const updateGalleryData = () => {
      ...
      setGalleryTopics(grouped);
    };

    const updateVideoData = () => {
      const grouped = vTopics.map(topic => {
        const topicVideos = videos.filter(v => (v as any).topicId === topic.id);
        topicVideos.sort((a, b) => (a.order ?? 999) - (b.order ?? 999) || (a.createdAt?.toMillis?.() || 0) - (b.createdAt?.toMillis?.() || 0));
        return {
          ...topic,
      ...
      setVideoTopics(grouped);
      
      // Set initial featured video if not set
      if (grouped.length > 0 && grouped[0].videos.length > 0) {
        const firstVideo = grouped[0].videos[0];
        setFeaturedVideo({
          id: firstVideo.id,
          type: 'video',
    ...
    const unsubscribeTopics = onSnapshot(collection(db, 'gallery'), (snapshot) => {
      topics = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }) as GalleryTopic);
      topics.sort((a, b) => (a.order ?? 999) - (b.order ?? 999));
      updateGalleryData();
    }, (error) => {
      console.error("Gallery topics fetch error:", error);
    });

    const unsubscribePhotos = onSnapshot(collectionGroup(db, 'photos'), (snapshot) => {
      photos = snapshot.docs.map(doc => ({
        id: doc.id,
        topicId: doc.ref.parent.parent?.id || '',
        ...doc.data()
      }) as any);
      photos.sort((a, b) => {
    ...
    const unsubscribeVideoTopics = onSnapshot(collection(db, 'videoGallery'), (snapshot) => {
      vTopics = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }) as VideoTopic);
      vTopics.sort((a, b) => (a.order ?? 999) - (b.order ?? 999));
      updateVideoData();
    }, (error) => {
      console.error("Gallery video topics fetch error:", error);
    });

    const unsubscribeVideos = onSnapshot(collectionGroup(db, 'videos'), (snapshot) => {
      ...
      console.error("Gallery videos fetch error:", error);
    });

    return () => {
      unsubscribeTopics();
      unsubscribePhotos();
      unsubscribeVideoTopics();
      unsubscribeVideos();
    };
  }, []);

  const videoItems: GalleryItem[] = videoTopics.flatMap(topic => 
    ...
    <div className="pt-40 pb-24 px-6 max-w-7xl mx-auto">
      <header className="mb-16 text-center">
        ...
        <div className="flex justify-start mt-8">
          <select
            value={activeTab === 'video' ? selectedVideoTopicId : selectedPhotoTopicId}
            onChange={(e) => {
              if (activeTab === 'video') {
                setSelectedVideoTopicId(e.target.value);
              } else {
                setSelectedPhotoTopicId(e.target.value);
              }
            }}
            className="bg-transparent border border-lime text-white px-4 py-2 rounded-lg outline-none focus:ring-2 focus:ring-lime/50 transition-colors"
          >
            {activeTab === 'video' 
        ...
        {activeTab === 'video' ? (
          ...
          >
            ...
            <div className="space-y-20">
              {videoTopics
                .filter(topic => selectedVideoTopicId === 'all' || topic.id === selectedVideoTopicId)
                .map((topic) => (
                ...
                setFeaturedVideo(item);
                        ...
                        >
                          <div className="aspect-video relative overflow-hidden">
                            ...
                            <div className="absolute inset-0 bg-black/40 group-hover:bg-black/20 transition-colors duration-500 flex items-center justify-center">
                              <div className={cn(
                                ...
                                isFeatured ? "bg-lime text-forest scale-110" : "bg-white/20 backdrop-blur-md text-white group-hover:bg-white group-hover:text-black"
                              )}>
                          ...
                          <div className="p-5 bg-white/5">
                            <h3 className="text-sm font-medium line-clamp-1 opacity-80 group-hover:opacity-100 transition-opacity">{video.title}</h3>
                          </div>
                        </motion.div>
                      );
                    })}
          ...
          >
            {galleryTopics
              .filter(topic => selectedPhotoTopicId === 'all' || topic.id === selectedPhotoTopicId)
              .map((topic, topicIdx) => (
              <div key={topic.id} className="space-y-8">
                <div className="flex items-center gap-6">
                  <h2 className="text-3xl md:text-4xl serif italic">
                      ...
                      <div key={item.id} className="space-y-3">
                        ...
                        >
                          ...
                          <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-500 flex flex-col justify-end p-6">
                            <div className="space-y-2">
                              <h3 className="text-lg serif text-white">{topic.title}</h3>
                              <p className="text-[10px] text-white/60 tracking-widest uppercase">View Full Image</p>
                            </div>
                          </div>
                        </motion.div>
                        {photo.caption && (
                          <p className="text-white/80 text-sm font-medium px-2 line-clamp-2">{photo.caption}</p>
                        )}
                      </div>
                    );
                  })}
            ...
            {galleryTopics.length === 0 && (
              <div className="py-40 text-center opacity-40 serif italic">
                등록된 사진이 없습니다.
              </div>
            )}
          </motion.div>
        ...
        {selectedItem && (
          ...
          >
            ...
            <button 
              ...
              className="absolute left-6 top-1/2 -translate-y-1/2 w-16 h-16 rounded-full bg-white/5 hover:bg-white/10 text-white flex items-center justify-center transition-all hover:scale-110 z-50 hidden md:flex"
            >
              <ChevronLeft size={32} />
            </button>
            ...
            <button 
              ...
              className="absolute right-6 top-1/2 -translate-y-1/2 w-16 h-16 rounded-full bg-white/5 hover:bg-white/10 text-white flex items-center justify-center transition-all hover:scale-110 z-50 hidden md:flex"
            >
            ...
            <motion.div
              ...
              className="relative z-10 max-w-6xl w-full h-full flex flex-col items-center justify-center p-4 md:p-24"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="relative w-full h-full flex flex-col items-center justify-center gap-8">
                ...
                <div className="text-center max-w-2xl">
                  ...
                  <motion.button 
                    ...
                    onClick={() => setSelectedItem(null)}
                    className="bg-white/10 hover:bg-white/20 text-white px-12 py-3 rounded-full text-sm font-medium transition-all border border-white/10 backdrop-blur-md"
                  >
                    닫기
                  </motion.button>
                </div>
